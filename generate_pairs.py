...
def make_pair_block(t1: str, t2: str, label: str = "", notes: str = "") -> dict:
    return {
        "ticker1": t1,
        "ticker2": t2,
        "label": label or f"{t1} vs {t2}",
        "relationship": f"Statistical screening pair — {t1} vs {t2}. Thesis to be determined if cointegration confirmed.",
        "lead": "unknown",
        "lag_days": 0,
        "cointegrated": "unknown",
        "sentinel_goals": ["correlation_decay"],
        "leading_indicators": [],
        "notes": notes or f"Generated by generate_pairs.py on {datetime.now().strftime('%Y-%m-%d')}. Screen with monitor.py --screen before adding to production monitoring."
    }
...
def main():
    ...
    if args.random is not None:
        ...
        pairs = [make_pair_block(t1, t2) for t1, t2 in sample]
        ...
        pairs = [make_pair_block(t1, t2, f"{cat} pair: {t1} vs {t2}") for t1, t2 in raw]
        ...
        pairs = [make_pair_block(t1, t2, f"{cat1}/{cat2}: {t1} vs {t2}") for t1, t2 in raw]
        ...
        pairs = [make_pair_block(t1, t2) for t1, t2 in raw]
    ...
    print(f"  python monitor.py --screen {output}")
